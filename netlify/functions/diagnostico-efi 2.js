...
export default async (req) => {
  try {
    ...
    try {
      ...
      console.error('diagnostico-efi (autenticação):', e && e.stack || e);
      return json({ ok: false, etapa: 'autenticacao', vars, erro: e && e.message ? e.message : 'Falha ao autenticar na Efí.' }, 502);
    }
  } catch (e) {
    console.error('diagnostico-efi (inesperado):', e && e.stack || e);
    return json({ ok: false, etapa: 'inesperado', erro: (e && e.message) ? e.message : String(e) }, 500);
  }
};
